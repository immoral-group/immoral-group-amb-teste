...
**Rol de GSAP (pedido explícito del usuario):** `gsap.ticker.add()` como loop de render (mismo patrón que los otros heroes); `gsap.quickTo()` para suavizar la posición del cursor antes de calcular el empuje sobre los hilos; un `gsap.fromTo()` que anima un `drawProgress` de 0 a 1 al cargar la página, de forma que los hilos se "escriben" de izquierda a derecha (encaja temáticamente con una página de "historia"); y un `gsap.to(..., {repeat:-1, yoyo:true})` que hace respirar lentamente la amplitud de las ondas en reposo, mismo espíritu que la respiración de contraste ya usada en el hero de Automatización.

**Ajustes tras feedback del usuario en la misma tarea:** (1) el canvas pinta un `fillRect` negro sólido cada frame en vez de `clearRect` transparente, para que el fondo del hero quede negro puro sin que se transparenten los orbes de color del fondo global fijo de la página (que se mantienen intactos para el resto del sitio); (2) el color de los hilos pasó de azul/cian de marca a blanco/gris (`rgb(255,255,255)` / `rgb(156,163,175)` alternados), a pedido explícito; (3) se añadió un segundo `<div>` superpuesto entre el canvas y el texto (`bg-gradient-to-r from-black via-black/70 to-transparent`, `z-index:-5`, entre el `-z-10` del canvas y el texto sin position) para oscurecer el lado izquierdo (donde vive el título en desktop, `xl:text-left`) y garantizar contraste de lectura sin opacar el lado derecho donde se ven los hilos.

**Por qué:** petición explícita del usuario de rehacer el fondo del hero de esta página siguiendo el patrón de los heroes de servicios existentes, pero con algo creativo, interactivo y usando GSAP; con feedback de ajuste de color y contraste tras la primera verificación visual.
