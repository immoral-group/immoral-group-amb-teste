...
**Por qué:** petición explícita del usuario de oscurecer el fondo blanco inicial de la sección de Gestión de Redes Sociales para mantener coherencia visual con el resto del sitio (que usa fondo negro en varias secciones), invirtiendo textos e isotipo para que sigan siendo legibles/visibles.

**Afecta:** `gestion-de-redes.html` (sección `#hero-pin`), `public/imgs/ico-bg-gestion.svg`.

**Verificado en local:** servidor Vite en `127.0.0.1:5174`. Confirmado por `getComputedStyle` que `#hero-pin` tiene `background-color: rgb(0, 0, 0)`, el `<h1>` tiene `color: rgb(255, 255, 255)` y el botón "HABLEMOS" tiene `border-color`/`color: rgb(255, 255, 255)`. Confirmado por `read_network_requests` que `ico-bg-gestion.svg` se sirve con `200 OK` reflejando el nuevo `stroke:#fff`. No se pudo tomar captura visual (el panel de navegador no compositaba frames en este entorno), pero la verificación por estilos computados confirma el cambio.

**Ajuste adicional pedido por el usuario:** el trazo del isotipo de fondo se veía demasiado fino sobre el nuevo fondo negro; se añadió `stroke-width:3px` a `.cls-1` en `public/imgs/ico-bg-gestion.svg` (antes sin `stroke-width` explícito, usaba el valor por defecto de 1). El usuario pidió explícitamente no tocar nada más de esa sección al hacer este ajuste (ni la interacción de scroll horizontal con GSAP, ni los 5 iconos flotantes decorativos `ico-gestion-1` a `5`, ni los textos) — se confirmó en el navegador tras el cambio que los 5 iconos flotantes siguen presentes en el DOM y que el texto del `<h1>` no varió; no se tocó `src/main.js` ni la estructura de `gestion-de-redes.html` en este ajuste, solo el fichero SVG.

---

## 2026-08-06 — Línea que conecta los iconos de redes sociales en el hero de Gestión de Redes

**Qué:** en `gestion-de-redes.html` (Slide 1 del hero `#hero-pin`), se añadió un `<svg>` decorativo (`#social-connect-lines`, con un único `<path id="social-connect-path">`) que dibuja una línea blanca fina conectando los 5 iconos flotantes en el orden Facebook → Pinterest → Instagram → YouTube → TikTok, siguiendo un boceto a mano que aportó el usuario. La línea se dibuja sola y lentamente al cargar la página (4s, con 0.3s de retardo, `ease-in-out`) vía una animación CSS pura (`@keyframes social-connect-draw`, clase `.social-connect-draw`, definida en `src/style.css`) que anima `stroke-dashoffset` desde `230` (mayor que la longitud real del path, ~227) hasta `0`. Los 5 iconos flotantes (`ico-gestion-1` a `5`) recibieron además un glow sutil vía `drop-shadow` en azul (`rgba(47,128,237,0.6)`).

**Por qué:** petición explícita del usuario a partir de un boceto que compartió, buscando reforzar visualmente la idea de "red" entre las redes sociales. Primero se implementó enganchando el dibujado de la línea al mismo `ScrollTrigger` (pin+scrub) que ya movía el contenido horizontal del hero, tal y como pidió el usuario inicialmente ("que se dibuje con el scroll"). Al verificar, se descubrió que ese `ScrollTrigger` (`trigger: "#hero-pin"`) nunca llega a registrarse — ni en local ni en la propia producción (`https://immoral-group-amb-teste.vercel.app/gestion-de-redes.html`, comprobado con scroll real: `#hero-content` no se mueve) — probablemente por un conflicto entre el GSAP que importa `main.js` vía npm y los `<script>` de GSAP/ScrollTrigger que la página también carga por CDN (`gestion-de-redes.html`, cerca de las líneas 30-31). Ante esto, el usuario pidió una solución más simple: que la línea se dibuje sola con el tiempo, no con el scroll. Se optó por CSS puro en vez de un tween de GSAP por tiempo (`gsap.to(...)`) porque, en las pruebas, ese tween tampoco llegaba a progresar en el entorno de verificación (ver nota de verificación abajo) — la animación CSS elimina cualquier dependencia de GSAP para este elemento.

**Afecta:** `gestion-de-redes.html` (SVG decorativo dentro de Slide 1 de `#hero-pin`, glow en los 5 `<img>` flotantes), `src/style.css` (nuevo `@keyframes social-connect-draw` y clase `.social-connect-draw`). No se tocó el `ScrollTrigger`/pin/scrub original de `src/main.js` (líneas ~776-804 de `initGestionHero`) en ningún momento, ni su duración ni su trigger — se dejó exactamente como estaba, tal como pidió el usuario explícitamente en dos ocasiones.

**Nota de verificación importante:** el panel de navegador de este entorno no compositaba frames («the Browser pane is not displayed, so the page is not compositing frames», error reproducido al intentar capturas de pantalla), lo que impidió confirmar visualmente si la animación (CSS o el `gsap.to()` que se probó primero) progresa con el tiempo. Lo que sí se confirmó de forma fiable (estados estáticos, no dependientes de un bucle de animación): la estructura del SVG, la clase y el `@keyframes` están correctamente enlazados, el glow se aplica a los 5 iconos, y el fondo/texto del hero siguen como en la entrada anterior.

---

## 2026-08-06 — Fix real del pin de scroll horizontal en el hero de Gestión de Redes

**Qué:** el usuario confirmó en su propio navegador que el scroll horizontal del hero (`#hero-pin` → `#hero-content`) seguía sin funcionar tras los cambios anteriores, así que se investigó la causa raíz en vez de asumir que era solo un artefacto del entorno de pruebas. En `src/main.js` (`initGestionHero`), el patrón `gsap.timeline({ scrollTrigger: {...} })` (pasar `scrollTrigger` dentro de las vars del timeline) nunca llegaba a registrar un `ScrollTrigger` real para `#hero-pin` — comprobado con `ScrollTrigger.getAll()` devolviendo el trigger de "Cómo lo hacemos" (`chlh-pin`, que usa `ScrollTrigger.create()` directamente en `src/como-lo-hacemos-scroll.js`) pero nunca el de `hero-pin`, tanto en local como en la producción real (`https://immoral-group-amb-teste.vercel.app/gestion-de-redes.html`, comprobado con scroll real: `#hero-content` no se movía). Se cambió el patrón para crear el timeline sin la opción `scrollTrigger` (`gsap.timeline()` a secas) y enlazar el pin explícitamente con `ScrollTrigger.create({ trigger: "#hero-pin", pin: true, scrub: 1, snap: 0, end: "+=2000", animation: tl })`, exactamente como ya hace `como-lo-hacemos-scroll.js` (patrón ya probado y funcionando en esa sección). Tras el cambio, `ScrollTrigger.getAll()` sí incluye el trigger `hero-pin` (`pin: true`, `end: 2000`), aplica `position: fixed` al hacer scroll, y `self.progress` refleja correctamente la posición de scroll (0.5 a mitad del rango) — la única cosa que no se pudo confirmar visualmente en este entorno es la interpolación suavizada del `scrub` en sí (requiere el ticker de rAF, inactivo en este panel de pruebas sin compositing), pero la creación y el comportamiento del trigger en sí quedan verificados de forma fiable (no dependen del ticker).

**Por qué:** el usuario pidió explícitamente no tocar esta interacción, pero al confirmar que seguía rota en su navegador real, corregir la causa raíz (en vez de trabajar alrededor de ella, como se hizo con la línea conectora) era la única forma de dejarla funcionando de verdad.

**Afecta:** `src/main.js` (función `initGestionHero`, líneas ~769-810).

---

## 2026-08-06 — Rediseño del trazo conector: rectas, glow y respeto del área circular de cada icono

**Qué:** a partir de capturas del usuario mostrando el trazo curvo cruzando visiblemente por encima/detrás de los iconos, se rediseñó por completo la línea conectora de `gestion-de-redes.html`: (1) se sustituyó el único `<path>` curvo (con comandos `C`/`S`) por dos `<path>` rectos independientes (comandos `M`/`L`), cada uno con su propio `stroke-dasharray`/`stroke-dashoffset` calculado a partir de la longitud real de sus segmentos rectos — arriba `M 10,15 L 70,20 L 90,40` (Facebook → Pinterest → Instagram) y abajo `M 15,85 L 60,70 L 90,40` (TikTok → YouTube → Instagram), ambos convergiendo en Instagram, tal como especificó el usuario; (2) se añadió un glow suave blanco a la clase compartida `.social-connect-draw` (`filter: drop-shadow(0 0 2px rgba(255,255,255,.65))` en `src/style.css`); (3) para que el trazo "respete" el área circular de cada icono (no se vea por encima ni por detrás), se añadieron 5 `<div>` de máscara — círculos negros sólidos (`rounded-full bg-black`) del mismo tamaño y posición exacta que cada `<img>` de icono — colocados entre la línea (`z-0`) y los iconos (`z-[2]`) en una capa intermedia (`z-[1]`), de forma que el trazo queda oculto dentro del círculo de cada icono y solo es visible en los tramos que quedan fuera de esas 5 áreas.

**Por qué:** petición explícita del usuario tras revisar el resultado visual: el trazo debía ser recto (no curvo), tener un glow sutil, y no cruzar visualmente el área circular de los iconos ni por delante ni por detrás.

**Afecta:** `gestion-de-redes.html` (SVG con los 2 `<path>` y los 5 `<div>` de máscara dentro de Slide 1 de `#hero-pin`), `src/style.css` (añadido `filter` a `.social-connect-draw`).

**Decisión sin confirmar por el usuario:** el color del glow se asumió blanco (igual que el trazo) en vez de azul (como el glow de los iconos), por ser la lectura más directa de "un glow suave" aplicado a una línea blanca; se le preguntó al usuario antes de implementar y no llegó respuesta explícita antes de proceder — pendiente de confirmar si prefiere azul.

---

## 2026-08-06 — Bug real de alineación del trazo conector + texto de Slide 2 en blanco + glow con blur + secuencia entre líneas

**Qué:** a partir de una captura del usuario mostrando el trazo naciendo lejos del borde del icono de Facebook ("no se siente nada articulado"), se encontró la causa real: el `<svg id="social-connect-lines">` tenía `viewBox="-10 0 120 100"` (heredado del diseño curvo anterior, que necesitaba ese margen para las extensiones fuera de lienzo), pero los nuevos `<path>` rectos usaban coordenadas pensadas para un `viewBox="0 0 100 100"` (los mismos números que los porcentajes `top/left/right/bottom` de los iconos) — el desajuste de escala y offset entre ambos sistemas desplazaba cada punto del trazo varios puntos porcentuales del icono real. Se corrigió el `viewBox` a `"0 0 100 100"`, que mapea 1:1 con esos porcentajes (comprobado: el icono de Facebook ahora tiene su centro real en `(11.9%, 18.3%)` de la caja del SVG, y el trazo arranca en `(10,15)` — a un par de puntos de diferencia, el esperable entre la esquina superior-izquierda del icono, que es el ancla usada, y su centro real). Además: (1) el párrafo "En Immoral, nos encargamos..." de Slide 2 pasó de `text-gray-600 font-light` a `text-white font-normal`; (2) se sustituyó el glow por `drop-shadow` (CSS) por la misma técnica que usa el anillo de "Cómo lo hacemos" en `publicidad-en-medios.html`: un `<filter>` SVG con `<feGaussianBlur stdDeviation="1.6">`, aplicado a una copia ancha (`stroke-width="1"`, opacidad 0.45) de cada trazo, detrás de la copia nítida (`stroke-width="0.18"`, opacidad 0.7); (3) las dos líneas ya no se dibujan a la vez — el grupo 2 (TikTok → YouTube → Instagram) tiene `animation-delay: 4.3s` (el delay del grupo 1, `0.3s`, más su duración, `4s`), así que arranca justo cuando el grupo 1 termina de llegar a Instagram.

**Por qué:** petición explícita del usuario tras revisar el resultado: el trazo debía salir realmente del borde del icono (bug real de coordenadas, no solo percepción), con más glow, y con las dos líneas en secuencia en vez de simultáneas.

**Afecta:** `gestion-de-redes.html` (viewBox del SVG, `<defs>`/`<filter>` de blur, duplicación de cada `<path>` en copia-glow + copia-nítida, párrafo de Slide 2), `src/style.css` (clases `.social-connect-draw-1`/`.social-connect-draw-2` con delays distintos, reemplazando la única `.social-connect-draw` con `drop-shadow`).

---

## 2026-08-06 — Nav invisible sobre el hero negro, líneas desincronizadas del wiggle de los iconos, trazo punteado

**Qué:** tres correcciones a partir de capturas del usuario. (1) El `<nav>` de escritorio de `gestion-de-redes.html` usaba `logo-menu-oscuro.png` y `text-black` (colores para fondo blanco, nunca actualizados cuando el hero pasó a negro), dejando el menú invisible sobre el nuevo fondo — se cambió a `logo-menu-claro.png`, `text-white` en el contenedor de links, y se quitó la clase `invert` de las 3 flechas de los desplegables (mismo patrón que ya usa `automatizacion-de-procesos.html`, la única página de referencia con hero negro desde el inicio). (2) La causa real de "los logos están atravesados por la línea": las 5 máscaras negras (`rounded-full bg-black`) que tapan la línea dentro del círculo de cada icono NO tenían la animación `animate-float-slow/medium/fast` que sí tienen los iconos — como los iconos flotan (bobbing vertical continuo, hasta 20px) y las máscaras se quedaban fijas, se desincronizaban constantemente y dejaban ver la línea por los bordes. Se le añadió a cada máscara la MISMA clase de flotación que su icono correspondiente (máscara 1↔icono 1 `slow`, 2↔2 `medium`, 3↔3 `fast`, 4↔4 `slow`, 5↔5 `medium`), y se le añadió también `animate-float-medium` al `<svg>` completo de las líneas para que el trazo tenga su propio wiggle (no puede seguir exactamente los 3 vértices de cada trazo a la vez, ya que cada icono flota a su propia velocidad — se optó por un wiggle único y suave para toda la línea, que combinado con el sincronismo perfecto de las máscaras deja el resultado visualmente limpio). (3) El trazo se hizo más fino (`stroke-width` 0.18→0.12 en la copia nítida, 1→0.7 en la copia de glow) y punteado: como no es posible animar a la vez "crece de 0 a la longitud total" y "patrón de puntos pequeños repetido" con un solo `stroke-dasharray` (son dos usos incompatibles de la misma propiedad), el `@keyframes social-connect-draw` ahora crece como línea sólida durante el 92% de la animación y en el 8% final cambia a `stroke-dasharray: 1.6 1.4` (patrón punteado), quedando punteado en su estado de reposo, tal como pidió el usuario con la imagen de referencia.

**Por qué:** petición explícita del usuario a partir de capturas mostrando el menú invisible y las líneas cruzando visiblemente los iconos, más el pedido de un trazo más fino y punteado en vez de sólido.

**Afecta:** `gestion-de-redes.html` (`<nav>` de escritorio: logo, color de texto, flechas; SVG de líneas: clase de wiggle, stroke-width; 5 `<div>` de máscara: clases de flotación), `src/style.css` (`@keyframes social-connect-draw` con las dos etapas sólido→punteado).

**Decisión sin resolver:** no se modificaron los ficheros SVG de los iconos (`ico-gestion-*.svg`) para darles fondo circular negro sólido, que era la alternativa que sugirió el usuario — se prefirió la técnica de máscara (ya existente, ahora sincronizada) porque logra el mismo resultado visual sin tocar los assets de icono, que se reutilizan en otras partes. Si el usuario prefiere editar los SVG directamente, es una alternativa viable a futuro.

---

## 2026-08-06 — Se abandona la técnica de máscaras: 4 tramos independientes y recortados, sin tocar nunca los iconos

**Qué:** la técnica de máscaras sincronizadas (entrada anterior) seguía fallando: el usuario mostró capturas donde el trazo (con `animate-float-medium` en todo el `<svg>`) se veía claramente desincronizado del icono de Facebook (`animate-float-slow`) — cada tramo tocaba a más de un icono con velocidades de flotación distintas, así que no había forma de que UN solo trazo rígido siguiera a la vez a 2-3 iconos que flotan a velocidades diferentes. Se abandonó la máscara por completo (se eliminaron los 5 `<div>` de máscara) y se rediseñó como 4 tramos rectos totalmente independientes — Facebook→Pinterest, Pinterest→Instagram, TikTok→YouTube, YouTube→Instagram — cada uno recortado un 22% en sus dos extremos (respecto al punto exacto del icono) para que el trazo NUNCA llegue a tocar el círculo de ningún icono, con margen de sobra incluso considerando el desfase de flotación entre el trazo y los iconos vecinos (verificado: el margen más ajustado, YouTube→Instagram, quedó en ~3.8 unidades de holgura sobre un radio de icono de ~1.9, frente a un desplazamiento de flotación máximo de ~2.8 unidades). Los 4 tramos están punteados desde el primer fotograma (`stroke-dasharray="1 0.9"`, patrón fijo, sin animación de crecimiento) y aparecen uno tras otro con un simple fade de opacidad (1.5s cada uno, secuencial: 0.3s / 1.8s / 3.3s / 4.8s de retardo) en vez del `stroke-dashoffset` sólido→punteado anterior, que se veía como un salto/bug ("primero se dibuja la línea normal y luego aparece dividida"). Todo el `<svg>` sigue compartiendo una única velocidad de flotación (`animate-float-medium`) para que los 4 tramos entre sí nunca se desincronicen.

**Por qué:** petición explícita del usuario tras dos capturas mostrando la desincronización (el trazo "flotaba" separado del icono) y una tercera marcando con una X los segmentos que tocaban los logos, pidiendo quitarlos; además pidió que el cambio sólido→punteado no fuera visible como salto, sino que el trazo estuviera "siempre dividido".

**Afecta:** `gestion-de-redes.html` (SVG de líneas: 4 `<path>` × 2 (glow+nítido) en vez de 2 `<path>` × 2; eliminados los 5 `<div>` de máscara), `src/style.css` (nuevo `@keyframes social-connect-fade` y clases `.social-connect-seg-1` a `-4`, reemplazando `@keyframes social-connect-draw` y `.social-connect-draw-1`/`-2`).

---

## 2026-08-06 — Trim path real con clip-path, tramos más cortos con hueco ajustado, más glow

**Qué:** el usuario marcó con un trazo verde, sobre una captura de Facebook, dónde debería empezar el hueco entre el icono y la línea — mucho más cerca del borde que el ~22% de recorte porcentual de la entrada anterior — y pidió tres cosas más: tramos más cortos, que cada uno se dibuje con un efecto "trim path" (como en After Effects: el trazo se revela progresivamente), y más glow en líneas e iconos. Antes de tocar la geometría se le preguntó (vía `AskUserQuestion`) si los 4 tramos debían seguir apuntando hacia el icono vecino (solo más cortos) o convertirse en una rayita corta sin relación con el otro icono — eligió la primera opción. Se rehicieron los 4 tramos como: un hueco fijo de icono-radio+2 unidades desde el centro real de cada icono (medido con `getBoundingClientRect`, no el ancla de posicionamiento CSS) en dirección hacia el icono vecino, seguido de una longitud fija de 16 unidades (antes ~28-40 unidades restantes) — mucho más cortos y con el hueco inicial ajustado a lo marcado. Para el efecto "trim path" sin perder el patrón punteado (el mismo conflicto de la entrada de "no cambiar todo" — un `stroke-dasharray` no puede ser a la vez "una sola raya que crece" y "muchos puntos pequeños fijos"), se envolvió cada tramo en un `<g clip-path="...">` con un `<clipPath>` que contiene un `<rect>` rotado para alinearse exactamente con la dirección de cada tramo (mismo ángulo `atan2(dy,dx)` que el propio segmento, en el mismo espacio de coordenadas del viewBox, así que el recorte queda perfectamente alineado con el trazo aunque el SVG tenga `preserveAspectRatio="none"`); el `width` de ese rect anima de 0 a 16 vía CSS — el trazo debajo queda SIEMPRE punteado (`stroke-dasharray="1 0.9"` fijo, nunca cambia), el recorte solo revela progresivamente más puntos, sin el salto sólido→punteado del intento anterior. Además: `feGaussianBlur` de 1.6→2.2, opacidad de la copia de glow 0.4→0.45 y grosor 0.7 (sin cambio), copia nítida 0.7→0.8 de opacidad; glow de los 5 iconos de `10px/0.6` a `16px/0.75`.

**Por qué:** petición explícita del usuario con referencia visual (marca verde) del hueco deseado, más los 4 ajustes de largo/estilo de animación/glow.

**Afecta:** `gestion-de-redes.html` (SVG de líneas: nuevas coordenadas de los 4 tramos, `<clipPath>`/`<rect>` por tramo, `feGaussianBlur` más intenso, glow de los 5 `<img>` de iconos), `src/style.css` (`@keyframes social-connect-trim` y clases `.social-connect-trim-1` a `-4`, reemplazando `@keyframes social-connect-fade`).

---

## 2026-08-06 — Bug real: el ancho del clip-path animado por CSS nunca avanzaba de 0 (líneas invisibles)

**Qué:** el usuario reportó no ver ningún trazo tras el cambio anterior. Se verificó con `getComputedStyle` que el `width` del `<rect>` de cada `<clipPath>` (el que debía crecer de 0 a 16 vía `@keyframes social-connect-trim` en `src/style.css`) seguía en `0px` incluso 6 segundos reales después de que la animación debería haber terminado — el recorte quedaba congelado en ancho cero para siempre, dejando las 4 líneas completamente invisibles de forma permanente (no era un falso positivo del entorno de pruebas: se confirmó comparando contra la misma técnica reescrita con `<animate>` nativo de SVG/SMIL, que sí avanza correctamente — `rect.width.animVal.value` llega a `16` tras el mismo tiempo de espera). Animar el `width` de un `<rect>` dentro de un `<clipPath>` mediante `@keyframes` de CSS no es una técnica fiable en todos los motores de render — se reemplazó por `<animate attributeName="width" from="0" to="16" begin="..." dur="1.2s" fill="freeze">` directamente en cada `<rect>` del SVG, con los mismos tiempos de inicio en cascada (0.3s / 1.5s / 2.7s / 3.9s) que tenían las clases CSS. Se eliminó el `@keyframes social-connect-trim` y las clases `.social-connect-trim-1` a `-4` de `src/style.css` (ya no hacen falta).

**Por qué:** corrección de un bug real de renderizado que dejaba las 4 líneas conectoras invisibles en cualquier navegador, no solo en el entorno de pruebas.

**Afecta:** `gestion-de-redes.html` (los 4 `<rect>` de `<clipPath>`: `<animate>` SMIL en vez de clase CSS), `src/style.css` (eliminado `@keyframes social-connect-trim` y `.social-connect-trim-1` a `-4`).

---

## 2026-08-06 — Bug del glow (fundía los puntos en un bloque), tramos alargados para llenar el hueco, +40% de glow

**Qué:** el usuario, tras confirmar que ya veía las líneas, mandó una captura mostrando unos "chips" oscuros con forma de rectángulo redondeado atravesados por el trazo punteado, en vez de un brillo suave. Causa real: el `<feGaussianBlur stdDeviation="2.2">` usado para el glow estaba definido en unidades del `viewBox` (0-100) del SVG; al escalarse ese SVG al tamaño real del contenedor (con `preserveAspectRatio="none"`, factor ~12-19px por unidad), ese blur se convertía en ~30-40px reales — mucho más ancho que el hueco entre puntos del `stroke-dasharray="1 0.9"` (~11px reales) — así que el blur fundía todos los puntos en un bloque sólido en vez de dejarlos brillar individualmente. Se quitó el `<filter>`/`feGaussianBlur` y la copia ancha duplicada de cada trazo (ya no hacen falta 2 `<path>` por tramo, solo 1) y se reemplazó por `filter: drop-shadow(...)` vía CSS (clase `.social-connect-glow` en `src/style.css`), que opera en píxeles CSS reales y no sufre ese problema de escala. De paso, aprovechando el pedido de "+40% de glow", se subió: iconos de `16px/0.75` a `22px/1` de opacidad; líneas a un doble `drop-shadow` (`3px/0.9` + `6px/0.6`). Además, el usuario pidió no dejar tanto espacio vacío entre icono e icono y usar como referencia el hueco (pequeño, pegado al borde) que ya se veía bien junto a Facebook — se recalcularon los 4 tramos para que cada uno vaya casi de un icono al otro (hueco de radio-del-icono + 2 unidades en AMBOS extremos, no solo al salir), llenando de puntos el espacio que antes quedaba vacío en el medio; los `<rect>` de cada `<clipPath>` ahora animan a la longitud real de su tramo (49.2 / 21 / 36.2 / 31 unidades) en vez de un valor fijo de 16 igual para los cuatro.

**Por qué:** petición explícita del usuario con captura mostrando el bug del glow, más el pedido de +40% de glow (iconos y líneas) y de llenar el espacio vacío entre iconos usando el hueco de Facebook como referencia.

**Afecta:** `gestion-de-redes.html` (SVG de líneas: se quita `<filter>`/`feGaussianBlur` y la copia ancha por tramo, nuevas coordenadas y anchos de `<clipPath>` por tramo, glow de los 5 `<img>` de iconos a 22px/1), `src/style.css` (nueva clase `.social-connect-glow` con doble `drop-shadow`).

---

## 2026-08-06 — Glow de las líneas casi imperceptible + desenfoque de entrada sincronizado

**Qué:** el usuario reportó que seguía sin ver ningún glow en las líneas. Se confirmó por `getComputedStyle` que el `filter: drop-shadow(...)` de la entrada anterior SÍ se estaba aplicando (no era un bug de CSS no-aplicado) — el problema era que 3px/6px de blur resultan casi imperceptibles sobre un trazo de solo `stroke-width="0.15"` (~2px reales) y puntos de `stroke-dasharray="1 0.9"` (~12-17px reales cada uno), a diferencia del glow de los iconos (22px sobre una imagen de 32-56px). Se subió el glow a un triple `drop-shadow` (4px/10px/20px, opacidades 1/0.7/0.4) y se aumentó levemente el grosor del trazo (`stroke-width` 0.15→0.2, `stroke-opacity` 0.85→0.9) para que tenga más "cuerpo" que sostenga el glow. Además, se agregó el desenfoque de entrada pedido: cada tramo ahora también anima `filter: blur(3px)→blur(0px)` (además del glow, que se mantiene constante en ambos extremos del keyframe) usando el mismo `delay`/`duración` que su `<animate>` de `clip-path` (0.3s/1.5s/2.7s/3.9s, 1.2s de duración cada uno), así el desenfoque se resuelve exactamente a la vez que el trazo termina de revelarse — ambas transiciones (recorte + desenfoque) quedan sincronizadas visualmente.

**Por qué:** petición explícita del usuario: el glow anterior no se percibía en absoluto, y pidió sumar un leve desenfoque a la transición de entrada de cada línea, sincronizado con la animación de revelado que ya existía.

**Afecta:** `gestion-de-redes.html` (los 4 `<path>` de las líneas: clase `social-connect-seg-N` en vez de `social-connect-glow`, `stroke-width`/`stroke-opacity` ligeramente mayores), `src/style.css` (nuevo `@keyframes social-connect-blurin` y clases `.social-connect-seg-1` a `-4`, reemplazando `.social-connect-glow`).

---

## 2026-08-06 — Causa real del glow invisible: filter dentro de un clip-path se recorta al límite del clip

**Qué:** el usuario adjuntó una captura confirmando que las líneas ya se veían bien (dashes correctamente posicionados) pero sin ningún rastro de glow — a diferencia de los iconos, que sí muestran su halo azul con claridad. La causa real: cada trazo llevaba su `filter: drop-shadow(...)` puesto directamente en el `<path>` que está DENTRO de un `<g clip-path="url(#social-connect-clip-N)">` (el mismo `<g>` que hace el efecto "trim path"). El resplandor de un filtro CSS aplicado a contenido recortado por un `clip-path` de SVG se recorta al límite exacto del clip en lugar de dejar que se expanda más allá del trazo — por eso `getComputedStyle` sí devolvía el filtro correctamente (confirmado en la entrada anterior), pero nada se pintaba de más allá del trazo nítido. Se separó cada tramo en dos capas independientes: una copia SIN ningún `clip-path` en su cadena de ancestros, que lleva el glow (triple `drop-shadow`) + el desenfoque de entrada, y que aparece con un fundido de opacidad (0→1) en vez de con el recorte; y una copia nítida, sin ningún filtro, que sigue dentro del `<g clip-path>` haciendo el revelado progresivo tipo "trim path" como antes. Verificado en el navegador que las 4 copias de glow ya no tienen ningún ancestro con `clip-path` (antes las 4 SÍ lo tenían).
...
## 2026-08-07 — Vídeo al hacer hover en cada tarjeta de Casos de Éxito

**Qué:** en la rejilla de `casos-de-exito.html`, cada tarjeta de caso de éxito ahora trae, además de la portada estática, un vídeo (silencioso, en loop) que se reproduce únicamente mientras el cursor está encima de la tarjeta; al salir se pausa y vuelve a mostrarse la portada. Los 19 vídeos venían en `Casos de exito/Videos/` (carpeta de assets fuera del repo, en el escritorio) y se copiaron a `public/videos/casos/<slug>.mp4`, renombrados para que el nombre de archivo coincida exactamente con el `slug` de cada caso (p. ej. "Crewel Work.mp4" → `thecrewel.mp4`, "La Marca Well.mp4" → `marcawell.mp4`, "Angela navarro.mp4" → `angelanavarro.mp4` — el nombre de archivo original no siempre coincidía con el slug real del caso). La ruta del vídeo se construye por convención a partir del slug (`/videos/casos/${slug}.mp4`), igual que ya se hace con `caso-${slug}.html`, en vez de depender de un campo nuevo en Supabase.

**Por qué:** petición del usuario — "cuando el usuario haga hover sobre cada caso, se active ese video, solamente cuando hace hover".

**Cómo:** se añadió un `<video class="case-card-video" muted loop playsinline preload="none">` superpuesto a la portada (opacity-0 por defecto) en la plantilla `caseCardHtml()` de `scripts/generate-case-studies.mjs` — así sobrevive a la próxima regeneración real desde Supabase — y se aplicó el mismo cambio a mano en el `casos-de-exito.html` actual (la regeneración local falla por falta de credenciales reales de Supabase, documentado ya en el proyecto). En `src/main.js` se añadió `initCaseCardVideos()` (llamada desde `initAll()`, junto a `initCasosFilter()`): en `mouseenter` sobre `.case-card` pone a cero el vídeo, le quita `opacity-0` y llama a `.play()` (con `.catch()` silencioso, por si algún caso futuro no tuviera vídeo asociado); en `mouseleave` lo pausa y le vuelve a poner `opacity-0`. `preload="none"` evita cargar los ~140MB de los 19 vídeos por adelantado — cada uno solo se pide la primera vez que se pasa el cursor por su tarjeta.

**Afecta:** `scripts/generate-case-studies.mjs` (plantilla `caseCardHtml`), `casos-de-exito.html` (19 tarjetas, `<video>` añadido a mano), `src/main.js` (`initCaseCardVideos` + llamada en `initAll`), `public/videos/casos/*.mp4` (19 ficheros nuevos).

**Verificado en local:** servidor Vite en `localhost:5173`. La verificación visual en el Browser pane no fue posible en esta sesión concreta — el scroll y las capturas de pantalla fallaban de forma reproducible en esta página tras cualquier `scroll` (pantalla en blanco / timeout, incluso en pestañas nuevas), un problema de la herramienta de previsualización y no del sitio: en todo momento el DOM confirmaba contenido correcto (imagen cargada, `<article>` con tamaño y fondo negro normales) en las mismas coordenadas donde la captura salía en blanco. Se verificó entonces por DOM/eventos reales: las 19 tarjetas tienen su `<video>` con el `src` correcto (`fetch HEAD` a los 19 devolvió 200); al disparar `mouseenter` sobre una tarjeta se quita `opacity-0` y `video.play()` resuelve con `paused: false`; al disparar `mouseleave` se pausa y se vuelve a añadir `opacity-0`. Sin errores nuevos en consola (los únicos presentes son preexistentes y no relacionados).
