...
- **`@ybouane/liquidglass` (npm) — evitar:** probado en 2026-08-04 para un efecto de vidrio real (WebGL) en `nuestra-historia.html`/`equipo.html`. Descartado por dos bugs: (1) su `postinstall` ejecuta `patch-package` sin declararlo como dependencia, lo que rompe `npm install` para cualquiera que clone el repo si no se añade `patch-package` como devDependency; (2) la captura de fondo (`html-to-image`) puede devolver un panel sólido blanco sin lanzar error ni warning, y re-inicializar el efecto (necesario en este sitio por su navegación tipo SPA, ver `updateDOM`/`initAll` en `src/main.js`) cuelga la promesa indefinidamente. El efecto de vidrio actual del sitio es CSS puro (clase `.liquid-glass` en `src/style.css`: `backdrop-filter` + filtro SVG `feDisplacementMap` + aberración cromática falsa vía pseudo-elementos). Ver entrada del 2026-08-04 en `.claude/TASK-LOG.md`.
